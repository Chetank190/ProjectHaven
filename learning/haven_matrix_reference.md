...
| Fallback LLM | **Llama 3.1 8B Instruct** via NVIDIA NIM container | Official NIM, zero code change to swap (same OpenAI API endpoint) |
...
| LLM container | NVIDIA NIM (`nvcr.io/nim/meta/llama-3.1-8b-instruct`) | Official NVIDIA inference microservice; same OpenAI API as llama.cpp |
...
> "Four: cuDF for GPU DataFrames, cuML for the KNN solver, NVIDIA NIM for Llama 3.1 inference (as the LLM fallback), and Nemotron-3-Nano-30B run via llama.cpp which targets the GB10 CUDA backend. RAPIDS comes from the `rapidsai/base:25.06-cuda12-py3.12` container. NIM runs as a separate Docker service with the official `nvcr.io/nim` image."
